...
function TrackApplication() {
  ...
  const handleChange = (id, value) => {
    axios({
      method: "post",
      url: "/api/adminHome/changingStatus",
      data: {
        id,
        value,
      },
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: "/api/adminHome/allApplication",
  ...
  const viewDeteils = async (id) => {
    const res = await axios.get(`/api/adminHome/viewDetails/${id}`);
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-center">
        <Button className="mb-5" onClick={() => navigate("/admin")}>
          Back
        </Button>
      </div>
